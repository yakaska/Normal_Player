...
public class AlbumArtCache {
    ...
    public void fetch(final String artUrl, final FetchListener listener) {
        ...
        new AsyncTask<Void, Void, Bitmap[]>() {
            ...
            protected Bitmap[] doInBackground(Void[] objects) {
                Bitmap[] bitmaps;
                try {
                    Bitmap bitmap = Picasso.get().load(artUrl).resize(MAX_ART_WIDTH, MAX_ART_HEIGHT).onlyScaleDown().get();
                    Bitmap icon = Bitmap.createScaledBitmap(bitmap, MAX_ART_WIDTH_ICON, MAX_ART_HEIGHT_ICON, false);
    ...
    public static abstract class FetchListener {
        ...
        public void onError(String artUrl, Exception e) {
            Log.e("Cache", "Failed to load from" + artUrl);
        }
    }
